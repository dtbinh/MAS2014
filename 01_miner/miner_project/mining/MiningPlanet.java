...
public class MiningPlanet extends jason.environment.Environment {
        ...
        if (sleep > 0) {
         ...
         try {
            synchronized (model) {
	            ...
	            actionCounter ++;
	            if (model.isAllGoldsCollected() && model.getInitialNbGolds() != 0 && !resultReported) {
	            	resultReported  = true;
	            	System.out.println();
	            	System.out.println("########################################################################");
	            	System.out.println(">>> All gold collected after " + actionCounter + " actions executed.");
	            	System.out.println("########################################################################");
	            	System.out.println();
	            }
    ...
    synchronized public List<Literal> getPercepts(String agName) {
    	return super.getPercepts(agName);
    }

    synchronized private void updateAgPercept(int ag) {
        updateAgPercept("miner" + (ag + 1), ag);
    }

    private void updateAgPercept(String agName, int ag) {
        clearPercepts(agName);
        addPercept(agName, Literal.parseLiteral("name(" + agName + ")"));
    ...
    private void updateAgPercept(String agName, int x, int y) {
        ...
        if (model.hasObject(WorldModel.OBSTACLE, x, y)) {
            ...
            if (model.hasObject(WorldModel.DEPOT, x, y)) {
                addPercept(agName, Literal.parseLiteral("cell(" + x + "," + y + ",depot)"));
            }
        }
    }
