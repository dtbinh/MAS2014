...
public class WorldModel extends GridWorldModel {
    ...
    boolean pick(int ag) {
        ...
        if (hasObject(WorldModel.GOLD, l.x, l.y)) {
            if (!isCarryingGold(ag)) {
            	if (hasFriendToHelpNearby(l.x, l.y)) {
	                remove(WorldModel.GOLD, l.x, l.y);
	                setAgCarryingGold(ag);
	                return true;
            	} else {
            		logger.warning("Agent " + (ag + 1) + " is trying the pick gold, but it needs another agent in the neighboring row to help!");
            	}
            } else {
	...
	private boolean hasFriendToHelpNearby(int x, int y) {
		if (inGrid(x+1, y) && hasObject(WorldModel.AGENT, x+1,y)) {
			return true;
		}
		...
		if (inGrid(x, y-1) && hasObject(WorldModel.AGENT, x,y-1)) {
			return true;
		}
